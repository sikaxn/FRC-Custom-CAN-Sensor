...
def send_mode_255():
    arb_id = make_can_msg_id(DEVICE_TYPE_ID, MANUFACTURER_ID, GENERAL_API, DEVICE_NUMBER)
...
def send_pixel_batch(batch, base_api=0x351):
    for i, (index, r, g, b) in enumerate(batch):
        api_id = base_api + i
        arb_id = make_can_msg_id(DEVICE_TYPE_ID, MANUFACTURER_ID, api_id, DEVICE_NUMBER)
        data = [
            (index >> 8) & 0xFF,
            index & 0xFF,
            r, g, b,
...
def read_led_info():
    ...
    while True:
        ...
        if msg and msg.arbitration_id == make_can_msg_id(DEVICE_TYPE_ID, MANUFACTURER_ID, ESP_FEEDBACK_API, DEVICE_NUMBER):
