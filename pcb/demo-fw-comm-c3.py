...
API_RX_CONTROL  = 0x185  # PC -> ESP32: [R,G,B,aux,0,0,0,0]
...
class CANTester(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Full Demo FW CAN Tester")
        ...
        self.aux_on = tk.BooleanVar(value=False)
        self.led_mode = tk.StringVar(value="manual")
        self.rainbow_cycle_ms = tk.IntVar(value=3000)
        ...
        # periodic senders
        self.after(100, self._send_185_periodic)  # RGB/aux sender
    ...
    def _build_ui(self):
        ...
        mode_frame = ttk.LabelFrame(self, text="LED Mode")
        mode_frame.pack(fill="x", **pad)

        mode_choice_row = ttk.Frame(mode_frame)
        mode_choice_row.pack(fill="x", padx=8, pady=6)
        ttk.Radiobutton(mode_choice_row, text="Manual RGB", value="manual",
                        variable=self.led_mode, command=self._refresh_preview).pack(side="left")
        ttk.Radiobutton(mode_choice_row, text="Rainbow", value="rainbow",
                        variable=self.led_mode, command=self._refresh_preview).pack(side="left", padx=8)

        speed_row = ttk.Frame(mode_frame)
        speed_row.pack(fill="x", padx=8, pady=6)
        ttk.Label(speed_row, text="Rainbow cycle (ms):", width=18).pack(side="left")
        speed_scale = ttk.Scale(speed_row, from_=250, to=10000, variable=self.rainbow_cycle_ms,
                                orient="horizontal", command=self._on_rainbow_speed_change)
        speed_scale.pack(side="left", fill="x", expand=True, padx=6)
        self.rainbow_speed_label = ttk.Label(speed_row, text=f"{self.rainbow_cycle_ms.get()} ms", width=10)
        self.rainbow_speed_label.pack(side="right")

        aux_frame = ttk.Frame(self)
        aux_frame.pack(fill="x", **pad)
        ttk.Button(aux_frame, text="Toggle Aux Byte", command=self._toggle_aux, width=18).pack(side="left")
        self.aux_label = ttk.Label(aux_frame, text="Aux: OFF", foreground="red")
        self.aux_label.pack(side="left", padx=12)
        ttk.Label(aux_frame, text="Sent in byte 3 of 0x185; full-demo-fw ignores it for hardware").pack(side="left", padx=8)

        preview = ttk.LabelFrame(self, text="Active LED Output")
        preview.pack(fill="x", **pad)
        self.preview_text = ttk.Label(preview, text="Mode: Manual  RGB=(0,0,0)")
        self.preview_text.pack(anchor="w", padx=8, pady=4)
        self.preview_swatch = tk.Label(preview, text="      ", bg="#000000", relief="sunken")
        self.preview_swatch.pack(anchor="w", padx=8, pady=(0, 8))
    ...
    def _toggle_aux(self):
        self.aux_on.set(not self.aux_on.get())
        self.aux_label.configure(text=f"Aux: {'ON' if self.aux_on.get() else 'OFF'}",
                                 foreground=("green" if self.aux_on.get() else "red"))

    def _on_rainbow_speed_change(self, _value):
        cycle_ms = max(250, min(10000, int(float(self.rainbow_cycle_ms.get()))))
        self.rainbow_cycle_ms.set(cycle_ms)
        self.rainbow_speed_label.config(text=f"{cycle_ms} ms")
        self._refresh_preview()

    def _get_manual_rgb(self):
        r = max(0, min(255, int(self.r_val.get())))
        g = max(0, min(255, int(self.g_val.get())))
        b = max(0, min(255, int(self.b_val.get())))
        return r, g, b

    def _get_rainbow_rgb(self):
        cycle_ms = max(250, int(self.rainbow_cycle_ms.get()))
        phase = (time.monotonic() * 1000.0) % cycle_ms
        hue = phase / float(cycle_ms)
        r_f, g_f, b_f = colorsys.hsv_to_rgb(hue, 1.0, 1.0)
        return int(r_f * 255), int(g_f * 255), int(b_f * 255)

    def _get_active_rgb(self):
        if self.led_mode.get() == "rainbow":
            return self._get_rainbow_rgb()
        return self._get_manual_rgb()

    def _refresh_preview(self):
        r, g, b = self._get_active_rgb()
        mode_name = "Rainbow" if self.led_mode.get() == "rainbow" else "Manual"
        self.preview_text.config(text=f"Mode: {mode_name}  RGB=({r},{g},{b})")
        self.preview_swatch.config(bg=f"#{r:02x}{g:02x}{b:02x}")

    # ------------------------
    # Periodic Senders
    # ------------------------
    def _send_185_periodic(self):
        """Send RGB + aux continuously; rainbow mode uses a faster cadence."""
        dn = max(0, min(63, int(self.device_number.get())))
        r, g, b = self._get_active_rgb()
        aux = 1 if self.aux_on.get() else 0
        data = bytes([r, g, b, aux, 0, 0, 0, 0])
        send_frame(API_RX_CONTROL, dn, data)
        self._refresh_preview()

        # Schedule next
        if self._run_threads:
            send_period_ms = 33 if self.led_mode.get() == "rainbow" else 100
            self.after(send_period_ms, self._send_185_periodic)
