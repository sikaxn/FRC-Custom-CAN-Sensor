...
constexpr gpio_num_t CAN_RX_PIN = GPIO_NUM_9;
constexpr uint8_t RFID_SS1_PIN = 1;
constexpr uint8_t RFID_SS2_PIN = 2;
constexpr uint8_t RFID_SCK_PIN = 4;
constexpr uint8_t RFID_MISO_PIN = 5;
constexpr uint8_t RFID_MOSI_PIN = 6;

// Addressable LED Strip
const uint16_t DEFAULT_NUM_LEDS = 140;
uint16_t NUM_LEDS = DEFAULT_NUM_LEDS;
CRGB leds[DEFAULT_NUM_LEDS];
constexpr uint8_t LED_DATA_PIN = 7;
...
// ==================  RFID DRIVERS  =================
// ==================================================
MFRC522DriverPinSimple ss_pin1(RFID_SS1_PIN);  // Reader 1 SS
MFRC522DriverPinSimple ss_pin2(RFID_SS2_PIN);  // Reader 2 SS
...
void TaskLEDWrite(void* pvParameters);            // Addressable LED animation task
...
volatile uint8_t  statusLedR = 0;
volatile uint8_t  statusLedG = 0;
volatile uint8_t  statusLedB = 0;
volatile bool     statusLedOn = false;
volatile bool     statusLedDirty = true;
...
void handleLEDCan(const twai_message_t& msg);
void sendLedFeedback();
void setStatusLedState(uint8_t r, uint8_t g, uint8_t b, bool on);
void showLeds();
void writeStatusLed();
BaseType_t createTaskCompat(TaskFunction_t task, const char* name, uint32_t stackDepth, void* parameters,
...
BaseType_t createTaskCompat(TaskFunction_t task, const char* name, uint32_t stackDepth, void* parameters,
  ...
  (void)coreId;
  return xTaskCreate(task, name, stackDepth, parameters, priority, taskHandle);
#else
  return xTaskCreatePinnedToCore(task, name, stackDepth, parameters, priority, taskHandle, coreId);
#endif
}
...
static inline void waitForCycle(uint32_t targetCycle) {
  while ((int32_t)(esp_cpu_get_cycle_count() - targetCycle) < 0) {
  }
}

static uint8_t scaleStatusComponent(uint8_t value) {
#ifdef RGB_BRIGHTNESS
  return (uint8_t)(((uint16_t)value * (uint16_t)RGB_BRIGHTNESS) / 255U);
#else
  return value;
#endif
}
...
#if defined(RGB_BUILTIN_LED_COLOR_ORDER) && (RGB_BUILTIN_LED_COLOR_ORDER == LED_COLOR_ORDER_RGB)
  const uint8_t colors[3] = {red, green, blue};
#elif defined(RGB_BUILTIN_LED_COLOR_ORDER) && (RGB_BUILTIN_LED_COLOR_ORDER == LED_COLOR_ORDER_BGR)
  const uint8_t colors[3] = {blue, green, red};
#elif defined(RGB_BUILTIN_LED_COLOR_ORDER) && (RGB_BUILTIN_LED_COLOR_ORDER == LED_COLOR_ORDER_BRG)
  const uint8_t colors[3] = {blue, red, green};
#elif defined(RGB_BUILTIN_LED_COLOR_ORDER) && (RGB_BUILTIN_LED_COLOR_ORDER == LED_COLOR_ORDER_RBG)
  const uint8_t colors[3] = {red, blue, green};
#elif defined(RGB_BUILTIN_LED_COLOR_ORDER) && (RGB_BUILTIN_LED_COLOR_ORDER == LED_COLOR_ORDER_GBR)
  const uint8_t colors[3] = {green, blue, red};
#else
  const uint8_t colors[3] = {green, red, blue};
#endif
  const uint32_t cyclesPerUs = ESP.getCpuFreqMHz();
  const uint32_t t0h = (cyclesPerUs * 4) / 10;   // 0.4 us
  const uint32_t t1h = (cyclesPerUs * 8) / 10;   // 0.8 us
  const uint32_t period = (cyclesPerUs * 125) / 100;  // 1.25 us

  taskDISABLE_INTERRUPTS();
  uint32_t cycle = esp_cpu_get_cycle_count();
  for (uint8_t color : colors) {
    for (uint8_t mask = 0x80; mask != 0; mask >>= 1) {
      const uint32_t highCycles = (color & mask) ? t1h : t0h;
      gpio_set_level((gpio_num_t)STATUS_LED_PIN, 1);
      cycle += highCycles;
      waitForCycle(cycle);
      gpio_set_level((gpio_num_t)STATUS_LED_PIN, 0);
      cycle += period - highCycles;
      waitForCycle(cycle);
    }
  }
...
void setup() {
  ...
  if (canRxQueue == nullptr) {
    ...
    createTaskCompat(TaskCANAlertHandler, "CAN Alert", 4096, NULL, 2, NULL, 0);
  ...
  // === Create Tasks ===
  if (!DISABLE_RFID) {
    createTaskCompat(TaskAutoBatteryManager, "Battery Manager", 4096, NULL, 1, NULL, 0);
  }
  createTaskCompat(TaskCANTx,            "CAN TX",         4096, NULL, 1, NULL, 0);
  createTaskCompat(TaskCANRx,            "CAN RX Unified", 4096, NULL, 1, NULL, 0);
  createTaskCompat(TaskLEDWrite,         "LED Task",       4096, NULL, 1, NULL, 0);
  createTaskCompat(TaskCANGlobalHandler, "Global Data",    4096, NULL, 1, NULL, 0);
  createTaskCompat(TaskLEDIndicator,     "LEDIndicator",   2048, NULL, 1, NULL, 0);
  createTaskCompat(TaskCANIDHelper,      "CANID Helper",   4096, NULL, 1, NULL, 0);
}
...
void TaskLEDWrite(void* pvParameters) {
  ...
  for (;;) {
    if (stripEnabled != canOnOff) {
      stripEnabled = canOnOff;
      if (stripEnabled) {
        modeRefresh = true;
      } else {
        fill_solid(leds, NUM_LEDS, CRGB::Black);
...
void TaskCANTx(void* pvParameters) {
  ...
  for (;;) {
    if (canAvailable) {
      ...
      msgMeta.data[0] = (year  >= 2000) ? (year - 2000) : 0;
      msgMeta.data[1] = month;
      msgMeta.data[2] = day;
      msgMeta.data[3] = hour;
      msgMeta.data[4] = minute;
      msgMeta.data[5] = batteryCycleCount;
      msgMeta.data[6] = currentState;
      msgMeta.data[7] = batteryNote;

      twai_transmit(&msgMeta, pdMS_TO_TICKS(10));

      // ==========================================================
      // 0x133 → Battery / robot status
      // ==========================================================
      ...
      msgStatus.data[0] = (uint8_t)readerDetected;
      msgStatus.data[1] = (uint8_t)heartbeatOnline;
      msgStatus.data[2] = (uint8_t)currentlyEnabled;
      msgStatus.data[3] = (uint8_t)pdOnline;
      msgStatus.data[4] = (uint8_t)javaOnline;
      msgStatus.data[5] = (uint8_t)batteryMetaValid;
      msgStatus.data[6] = (uint8_t)useRoboRIOEnergy;
      msgStatus.data[7] = (uint8_t)writeFailActive;

      twai_transmit(&msgStatus, pdMS_TO_TICKS(10));
      }

      if (now - lastLedFeedback >= 100) {
        lastLedFeedback = now;
        sendLedFeedback();
      }
    }
...
void handleLEDCan(const twai_message_t& msg) {
  ...
  if (api == GENERAL_API_1) {
    ...
    canBrig   = msg.data[4];
    canOnOff  = msg.data[5];
    canParam0 = msg.data[6];
    canParam1 = msg.data[7];
    if (newMode != canMode) {
      canMode = newMode;
      Serial.printf("[LED] CAN mode -> %u\n", canMode);
    } else {
      canMode = newMode;
    }
    modeRefresh = true;
