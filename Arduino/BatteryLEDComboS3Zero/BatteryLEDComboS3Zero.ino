...
#error "Enable Tools > USB CDC On Boot for ESP32-S3 USB serial."
#endif
...
// ==================================================
// RFID is disabled in this build. These are reserved, otherwise-unused S3 GPIOs
// for a future RFID-enabled build.
constexpr uint8_t RST_PIN = 13;
constexpr uint8_t RFID_SS1_PIN = 11;
constexpr uint8_t RFID_SS2_PIN = 12;
constexpr uint8_t RFID_SCK_PIN = 14;
constexpr uint8_t RFID_MISO_PIN = 15;
constexpr uint8_t RFID_MOSI_PIN = 16;

constexpr gpio_num_t CAN_TX_PIN = GPIO_NUM_9;
constexpr gpio_num_t CAN_RX_PIN = GPIO_NUM_10;

// The onboard BOOT button pulls GPIO 0 low. It cycles these LED modes.
constexpr uint8_t MODE_BUTTON_PIN = 0;
const uint8_t buttonModes[] = {
...
const uint16_t DEFAULT_NUM_LEDS = 8;
...
// Kept separate because the onboard pixel has a different protocol order than
...
void TaskLEDWrite(void* pvParameters) {
  ...
  for (;;) {
    ...
    if (lastModeButton == HIGH && modeButton == LOW) {
      ...
      canMode = buttonModes[currentModeIndex];
      modeRefresh = true;
