...
volatile bool rtcTimeAvailable = false;     // RTC valid flag
volatile bool rioTimeAvailable = false;     // RIO heartbeat time valid (only after ENABLED once)
volatile bool rtcUpdatedOnce = false;       // Have we synced DS3231 using RIO time?

// Time / Date
volatile int year = 0, month = 0, day = 0;
volatile uint8_t hour = 0, minute = 0, second = 0;
volatile int rioYear = 0, rioMonth = 0, rioDay = 0;
volatile uint8_t rioHour = 0, rioMinute = 0, rioSecond = 0;

volatile bool rtcTimeValid = false;       // RTC powered + trustworthy
volatile bool rioTimeValid = false;       // RIO enabled at least once

// ESP32 internal software clock
volatile bool espTimeValid = false;
volatile uint32_t espEpoch = 0;          // seconds since arbitrary epoch
volatile uint32_t espEpochStartMs = 0;   // when epoch was set
...
void setup() {
  ...
  Wire.begin(21, 22);
  if (!rtc.begin()) {
    Serial.println("[RTC] NOT FOUND → rtcTimeValid = false");
    rtcTimeValid = false;
  }
  else {
    if (rtc.lostPower()) {
      Serial.println("[RTC] Lost power → do NOT trust time until RIO enables");
      rtcTimeValid = false;       // VERY IMPORTANT
    } 
    else {
      rtcTimeValid = true;        // RTC OK
    }
  }
  ...
  for (;;) {
    switch (currentState) {
      ...
      case STATE_PARSE_AND_WRITE_INITIAL: {
        char tbuf[11] = "0000000000";

      if (rioTimeValid || rtcTimeValid) {
      ...
      writeNewUsageLog(currentSessionId, String(tbuf), 0, 0.0f, lockedReader);
...
void TaskCANGlobalHandler(void* pvParameters) {
  ...
  for (;;) {
   ...
   // =======================================================
  // SYSTEM TIME SELECTION LOGIC
  // =======================================================

  // Priority 1: RIO time (robot enabled once)
  if (rioTimeValid) {
      year   = rioYear;
      month  = rioMonth;
      day    = rioDay;
      ...
      // On first RIO-valid moment, sync RTC and ESP software clock
      if (!rtcTimeValid) {
          rtc.adjust(DateTime(year, month, day, hour, minute, second));
          rtcTimeValid = true;
      }

      setEspTime(year, month, day, hour, minute, second);
  }

  // Priority 2: RTC valid time (RIO not enabled yet)
  ...
  // Priority 3: ESP internal time
  else if (espTimeValid) {
      getEspTime(year, month, day, hour, minute, second);
  }

  // Priority 4: No time source → fallback 0
  else {
...
void handleHeartbeat(const twai_message_t& msg) {
  ...
  // === Decode datetime ===
  rioYear   = get_bits(bits, 26, 6) + 2000 - 36;
  rioMonth  = get_bits(bits, 22, 4) + 1;
  rioDay    = get_bits(bits, 17, 5);
  rioHour   = get_bits(bits, 0, 5);
  rioMinute = get_bits(bits, 5, 6);
  rioSecond = std::min(get_bits(bits, 11, 6), 59);
...
void setEspTime(int y, int m, int d, int hh, int mm, int ss) {
    // Convert calendar to seconds (naive, but works for our usage)
    tm t = {};
    t.tm_year = y - 1900;
    t.tm_mon  = m - 1;
    t.tm_mday = d;
    ...
    espEpoch = epoch;
    espEpochStartMs = millis();
    espTimeValid = true;
}

void getEspTime(int &y, int &m, int &d, int &hh, int &mm, int &ss) {
    if (!espTimeValid) {
        y = m = d = hh = mm = ss = 0;
        return;
    }

    uint32_t nowSec = espEpoch + (millis() - espEpochStartMs) / 1000;

    tm *t = localtime((time_t*)&nowSec);
