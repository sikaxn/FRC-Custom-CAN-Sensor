  ...
  for (;;) {
    ...
    if (twai_receive(&msg, pdMS_TO_TICKS(10)) == ESP_OK && msg.extd) {
      ...
      if (msg.identifier == HEARTBEAT_ID && msg.data_length_code == 8) {
        ...
        // === Extract FRC heartbeat bitstream ===
        uint64_t bits = 0;
        for (int i = 0; i < 8; ++i) {
          bits = (bits << 8) | msg.data[i];
        }

        // === Helper macro: extract bits like in Python ===
        #define GET_BITS(value, start, length) ((value >> (64 - (start + length))) & ((1ULL << (length)) - 1))
        ...
        second  = GET_BITS(bits, 11, 6);
        minute  = GET_BITS(bits, 5, 6);
        hour    = GET_BITS(bits, 0, 5);
int fullYear = 2000 + GET_BITS(bits, 26, 6) - 36;

        #undef GET_BITS

        if (firstRead || currentlyEnabled != wasEnabled) {
        Serial.printf(
          "[CAN] Robot %s | Voltage: %.1fV | Energy: %d | Lowest: %.2fV | Date: %04d-%02d-%02d %02d:%02d:%02d\n",
    ...
    if (!heartbeatCurrentlyOk && lastHeartbeatOk) {
      ...
      // No print — silent DISABLED fallback
    }
